...
class OmegaPreferences(val context: Context) : SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    var colorizedLegacyTreatment by BooleanPref(
    ...
    val customAppIcon = object : MutableMapPref<ComponentKey, IconPackManager.CustomIconEntry>(
        "pref_appIconMap",
        reloadAll
    ) {
        override fun flattenKey(key: ComponentKey) = key.toString()
        ...
        override fun unflattenValue(value: String) =
            IconPackManager.CustomIconEntry.fromString(value)
    }
        ...
        if (version != CURRENT_VERSION) {
            with(prefs.edit()) {
                ...
                putString(
                    ...
                    when (prefs.getString("pref_pulldownAction", "1")?.toInt()) {
